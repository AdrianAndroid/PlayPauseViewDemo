...
import com.flannery.playpauseviewdemo.BuildConfig;
import com.flannery.playpauseviewdemo.R;
...
//        mPaint.setStyle(Paint.Style.STROKE);
        ...
        if (mDirection == Direction.NEGATIVE.value) {
            ...
            if (BuildConfig.DEBUG) {
                Log.i(PlayPauseView.class.getSimpleName(), "NEGATIVE left右边->" + (barWidth + mRectLT) + " , right左边->" + (rightLeftTop + mRectLT));
            }
        } else {
            mLeftPath.moveTo(leftLeftTop + mRectLT, mRectLT); // 左上角
            mLeftPath.lineTo(mRectLT, mRectHeight + mRectLT); // 左下角
            mLeftPath.lineTo(barWidth + mRectLT, mRectHeight + mRectLT); // 右下角
            mLeftPath.lineTo(barWidth + mRectLT, mRectLT); // 右上角
            mLeftPath.close();

            mRightPath.moveTo(rightLeftTop + mRectLT, mRectLT); // 左上角
            mRightPath.lineTo(rightLeftTop + mRectLT, mRectHeight + mRectLT); // 左下角
            mRightPath.lineTo(rightLeftTop + mRectLT + barWidth, mRectHeight + mRectLT); // 右下角
            mRightPath.lineTo(rightRightBottom + mRectLT, mRectLT); // 右上角
            mRightPath.close();

            if (BuildConfig.DEBUG) {
                Log.i(PlayPauseView.class.getSimpleName(), "POSITIVE left右边->" + (barWidth + mRectLT) + " , right左边->" + (rightLeftTop + mRectLT));
            }
        }
        ...
        float top = mRectLT;
        float bottom = mRectLT + mRectHeight;

//        // 左边线 -> 右边
//        drawLine(canvas, barWidth + mRectLT, 0, barWidth + mRectLT, mHeight);
//        // 左边线 -> 左边
//        drawLine(canvas, rightLeftTop + mRectLT, 0, rightLeftTop + mRectLT, mHeight);

        // 边框线
        drawLine(canvas, 0, 0, 0, mHeight);
        drawLine(canvas, 0, 0, mWidth, 0);
        drawLine(canvas, 0, mHeight, mWidth, mHeight);
        drawLine(canvas, mWidth, mHeight, mWidth, 0);
        // 头部
        drawLine(canvas, 0, top, mWidth, top);
        // 底部
        drawLine(canvas, 0, bottom, mWidth, bottom);
